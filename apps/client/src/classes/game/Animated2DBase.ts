...
import { spine as SPINE } from "@src/utils/spine-threejs";
...
export class Animated2DBase {
	...
	public async load() {
		...
		this.animationNamesList =
			this.skeletonMesh.state.data.skeletonData.animations.map((animation) => animation.name) || [];
...
async function loadRoleModel(
...
): Promise<SPINE.threejs.SkeletonMesh> {
	...
	const skeletonMesh = new SPINE.threejs.SkeletonMesh(skeletonData, (parameters) => {
...
function loadTextureAtlas(assetManager: SPINE.threejs.AssetManager, path: string) {
	return new Promise((resolve, reject) => {
		assetManager.loadTextureAtlas(
			path,
			(path: string, atlas: SPINE.TextureAtlas) => {
				resolve(atlas);
			},
			(path: string, message: string) => {
				reject(message);
