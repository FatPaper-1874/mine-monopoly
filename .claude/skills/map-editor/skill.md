...
player.modifierManager.add({
    ...
    fn: async (command, context) => {
        ...
        if (hasSix) {
            await player.gain(100);
            gameProcess.msgNotifyBroadcast("success", "幸运加成！投出6，获得100元");
        }
    }
});
...
- `gameProcess.msgNotifyBroadcast(type, msg: string)` - 广播消息通知（type: "success" | "warning" | "error" | "info"）
...
(async (ctx: GameContext, gameProcess: IGameProcess) => {
    ...
    gameProcess.handlePlayerBuyProperty = async (player: IPlayer, property: IProperty) => {
        ...
        gameProcess.msgNotifyBroadcast("info", `${player.name} 买下了地皮 ${property.name}`);
        gameProcess.gameLogBroadcast(
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    ...
    // 3. 发送通知
    gameProcess.msgNotifyBroadcast("success", `${player.name} 获得了 1000 元！`);
    ...
    player.modifierManager.add({
        ...
        fn: async (cmd, ctx) => {
            ...
            gameProcess.msgNotifyBroadcast("success", `${player.name} 的护盾抵挡了 ${blockedAmount} 元伤害！`);
            ctx.cancel();
            ctx.setResult({ money: 0, target: cmd.payload.target });
        },
    });
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    ...
    gameProcess.msgNotifyBroadcast("success", `${player.name} 获得了一个十面骰子！`);
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    ...
    if (player.dices.length > 0) {
        ...
        gameProcess.msgNotifyBroadcast("info", `${player.name} 获得预言！下次投掷必然投出6`);
        gameProcess.gameLogBroadcast(`${player.name} 获得了骰子预言`);
    } else {
        gameProcess.msgNotifyBroadcast("error", `${player.name} 没有骰子，无法使用预言`);
    }
});
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    ...
    gameProcess.msgNotifyBroadcast("success", `${player.name} 获得了一个额外的骰子！`);
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    ...
    gameProcess.msgNotifyBroadcast("success", `${player.name} 的所有骰子升级为八面骰子！`);
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    ...
    gameProcess.msgNotifyBroadcast("success", `${player.name} 获得了${randomDice.name}！`);
    ...
    player.modifierManager.add({
        ...
        fn: async (cmd, ctx) => {
            ...
            if (reducedAmount > 0) {
                ...
                gameProcess.msgNotifyBroadcast("info", `${player.name} 的财富护盾生效，减免了 ${reducedAmount} 元！`);
            }
        },
    });
});
    ...
    player.modifierManager.add({
        ...
        fn: async (command, context) => {
            ...
            if (player.customData.prophecyCount % 3 === 0) {
                ...
                gameProcess.msgNotifyBroadcast("info", `${player.name} 的预言生效！下次投掷必然投出6`);
            }
    ...
    player.modifierManager.add({
        ...
        fn: async (command, context) => {
            if (player.customData.firstRoll) {
                ...
                gameProcess.msgNotifyBroadcast("success", `${player.name} 的双倍首掷生效！`);
            }
        }
    });
});
    ...
    player.modifierManager.add({
        ...
        fn: async (command, context) => {
            ...
            if (sixCount > 0) {
                ...
                gameProcess.msgNotifyBroadcast("success", `${player.name} 投出了${sixCount}个6，获得${bonus}元！`);
            }
        }
    });
});
...
(async (ctx: GameRoundStartContext, gameProcess: IGameProcess) => {
    ...
    for (const player of gameProcess.players.values()) {
        ...
        gameProcess.msgNotifyBroadcast("success", `${player.name} 获得回合工资 1000 元`);
    }
...
const event = await add_map_event({
    ...
    effectCode: `
        (async (player: IPlayer, gameProcess: IGameProcess) => {
            await player.gain(100);
            gameProcess.msgNotifyBroadcast("success", "测试：获得100元");
        });
    `,
    description: "测试用事件"
});
    ...
    effectCode: `
        (async (player: IPlayer, gameProcess: IGameProcess) => {
            player.modifierManager.add({
                ...
                fn: async (cmd, ctx) => {
                    gameProcess.msgNotifyBroadcast("info", `${player.name} 的护盾抵挡了 ${cmd.payload.money} 元伤害！`);
                    ctx.cancel();
                    ctx.setResult({ money: 0, target: cmd.payload.target });
                },
            });
    ...
    effectCode: `
        (async (sourcePlayer: IPlayer, target: IPlayer, gameProcess: IGameProcess) => {
            ...
            if (target.money >= amount) {
                ...
                gameProcess.msgNotifyBroadcast("success", \`抢夺成功！获得 \${amount} 元\`);
            } else {
                gameProcess.msgNotifyBroadcast("error", \`\${target.name} 的金钱不足，抢夺失败！\`);
            }
        });
    `
});
            ...
            sourcePlayer.modifierManager.add({
                ...
                fn: async (cmd, ctx) => {
                    ...
                    if (bonus > 0) {
                        ...
                        gameProcess.msgNotifyBroadcast("success", \`财富加倍生效！额外获得 \${bonus} 元\`);
                    }
                },
            });

            gameProcess.msgNotifyBroadcast("info", "财富加倍卡已使用！下一次收入将翻倍");
        });
    `
});
...
await add_chance_card({
    ...
    effectCode: `
        (async (sourcePlayer: IPlayer, target: IPlayer[], gameProcess: IGameProcess) => {
            ...
            gameProcess.msgNotifyBroadcast("success", \`征税成功！共获得 \${totalCollected} 元\`);
        });
    `
});
```
    ...
    initEventCode: `
        (async (ctx: GameRoundStartContext, gameProcess: IGameProcess) => {
            ...
            if (round % 5 === 0) {
                for (const player of gameProcess.players.values()) {
                    await player.gain(2000);
                    gameProcess.msgNotifyBroadcast("success", \`\${player.name} 获得5回合奖金 2000 元\`);
                }
            }
        });
    `
...
// 显示消息通知
gameProcess.msgNotifyBroadcast("info", `提示信息`);
    ...
    effectCode: `
        (async (player: IPlayer, gameProcess: IGameProcess) => {
            player.modifierManager.add({
                ...
                fn: async (cmd, ctx) => {
                    const blockedAmount = cmd.payload.money;
                    gameProcess.msgNotifyBroadcast(
                        "success",
...
await update_extra_libs({
    code: `
        ...
        export async function grantReward(player: IPlayer, amount: number, reason: string, gameProcess: IGameProcess) {
            ...
            gameProcess.msgNotifyBroadcast("success", \`\${player.name} \${reason}，获得 \${amount} 元！\`);
        ...
        export async function deductMoney(player: IPlayer, amount: number, reason: string, gameProcess: IGameProcess) {
            if (player.money < amount) {
                gameProcess.msgNotifyBroadcast("error", \`\${player.name} 金钱不足！\`);
                return false;
            }
            await player.cost(amount);
            gameProcess.msgNotifyBroadcast("info", \`\${player.name} \${reason}，支付 \${amount} 元\`);
            return true;
        }
    `
});
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    if (player.dices.length > 0) {
        // 为第一个骰子设置预言值为6
        player.dices[0].setProphecy(6);
        gameProcess.msgNotifyBroadcast("info", "下次投掷必然投出6！");
    }
});
...
player.modifierManager.add({
    ...
    fn: async (command, context) => {
        ...
        if (total >= 10) {
            ...
            gameProcess.msgNotifyBroadcast("success", `总点数${total}，获得500元！`);
        } else if (total <= 3) {
            await player.cost(200);
            gameProcess.msgNotifyBroadcast("error", `总点数${total}，扣除200元！`);
        }
        ...
        if (hasSix) {
            await player.gain(100);
            gameProcess.msgNotifyBroadcast("success", "投出了6，额外获得100元！");
        }
    }
});
```
...
(async (player: IPlayer, gameProcess: IGameProcess) => {
    if (player.dices.length > 1) {  // 至少保留一个骰子
        ...
        gameProcess.msgNotifyBroadcast("error", "至少需要保留一个骰子！");
    }
});
